...
class BoxesSketch : BaseSketch() {
    ...
    private val motionTranslateBWDBtn =
        galaxy.createPushButton(6, 5) { starMotion = Starfield.Motion.TRANSLATING_BACKWARD }
    private val motionTranslateFWDBtn =
        galaxy.createPushButton(6, 6) { starMotion = Starfield.Motion.TRANSLATING_FORWARD }
    ...
    override fun draw() = with(sketch) {
        ...
        starfield1.rotate(
            PApplet.map(
                ...
                50f,
                0f,
                0.04f * starfieldRotationPot.value
            ) + radians(starRotationZ)
        )
        starfield2.rotate(
            PApplet.map(
                bassSum,
                ...
                50f,
                0f,
                0.08f * starfieldRotationPot.value
            ) + radians(starRotationZ)
        )
